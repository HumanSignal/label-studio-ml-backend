...
def convert_to_ls(pixel_position, original_length):
    return 100 * pixel_position / original_length


def build_model_predictions(page_prediction, task):
    ...
    for layer in page_prediction.layers:
        for label in ["Material Description", "Depth Interval"]:
            if label == "Material Description":
                value = {
                    "x": convert_to_ls(
                        layer.material_description.rect.x0, page_prediction.page_width
                    ),
                    "y": convert_to_ls(
                        layer.material_description.rect.y0, page_prediction.page_height
                    ),
                    "width": convert_to_ls(
                ...
                if layer.depth_interval is None:
                    ...
                    layer.depth_interval.start is None
                    and layer.depth_interval.end is not None
                ):
                    layers_with_depth_intervals.append(layer.id.hex)
                    value = {
                        "x": convert_to_ls(
                            layer.depth_interval.end.rect.x0, page_prediction.page_width
                        ),
                        "y": convert_to_ls(
                    ...
                    layer.depth_interval.start is not None
                    and layer.depth_interval.end is not None
                ):
                    layers_with_depth_intervals.append(layer.id.hex)
                    value = {
                    ...
                    logger.warning(
                        f"Depth interval for layer {layer.id.hex} is not complete."
                    )
                    continue

            pre_annotation_result.extend(
                create_ls_result(layer, page_prediction, value, label)
            )
...
def create_ls_result(layer, page_prediction, value, label):
    ...
    for _type in types:
        ...
        pre_annotation["original_widht"] = int(
            page_prediction.page_width * 3
        )  # we used a scale factor of three
        pre_annotation["original_height"] = int(
            page_prediction.page_height * 3
        )  # we used a scale factor of three
            ...
            if label == "Material Description":
                ...
                if layer.depth_interval.start is None:
                    pre_annotation["value"]["text"] = [
                        f"start: 0 end: {layer.depth_interval.end.value}"
                    ]
                else:
                    pre_annotation["value"]["text"] = [
