...
label_configs = [
    # test 1: one control tag with brush labels
    """
    <View>
      <Image name="image" value="$image"/>
      <BrushLabels name="label" toName="image" model_score_threshold="0.6">
...
PICKLE_PATH = TEST_DIR + "/test_brush_labels.pickle"
...
# Mock YOLO prediction results to ensure deterministic brush points
# (brush point count varies across ultralytics versions)
yolo_results = get_yolo_results()

expected = [
    # test 1: one control tag with brush labels
    load_file(TEST_DIR + "/test_brush_labels.json")
]
...
def test_brushlabels_predict(client, label_config, task, yolo_result, expect):
    """Test brush labels prediction with mocked YOLO results.

    We mock YOLO.predict because brush point counts vary across ultralytics
    versions due to different mask-to-contour conversion algorithms.

    To regenerate the pickle and json fixtures, run:
        pytest tests/test_brush_labels.py::test_create_brush_labels_pickle -s
    """
    ...
    if yolo_result is not None:
        ...
        # Run test_create_brush_labels_pickle first to generate fixtures
        pytest.skip(
            "Pickle file not found. Run test_create_brush_labels_pickle first "
...
def test_create_brush_labels_pickle(client):
    """Generate pickle and json fixtures for brush labels test.
    ...
    2. Save the YOLO results to test_brush_labels.pickle
    3. Save the expected output to test_brush_labels.json
    """
    ...
    # Save expected JSON
    json_path = TEST_DIR + "/test_brush_labels.json"
    ...
    print(
        "You can now run: pytest tests/test_brush_labels.py::test_brushlabels_predict"
    )
