...
- `DOCLING_INCLUDE_TABLE_STRUCTURE` — for every `TableItem`, rebuild the interface's expected table overlays from `TableData.table_cells`: `table_row` strips at each grid row (full table width), `table_column` strips at each grid column (full table height), `table_merged_cell` overlays at cells with `row_span` or `col_span` > 1, semantic overlays (`column_header` / `row_header` / `row_section`) at their per-cell geometry, and a `text` content child per non-empty cell. All children are `parentId`-linked to the enclosing table. The row/column strips OVERLAP by design — the DocLang emitter computes each cell as `intersect(row_bbox, col_bbox)` and assigns text content to the origin cell by bbox overlap, so the DocLang round-trip rebuilds `<fcel/>` markup from the geometry alone. Set to `false` to keep tables as a single flat rect.
- `DOCLING_INCLUDE_RELATIONS` — emit `to_caption` / `to_footnote` / `to_value` linking polylines from `FloatingItem.captions` / `FloatingItem.footnotes` / `KeyValueItem.graph.links[TO_VALUE]`, plus `merge` polylines for every `InlineGroup` (Docling's representation of one logical element split across columns / pages / line breaks; the DocLang emitter maps a `merge` polyline to a shared `thread_id`). Without these, captions become detached free-floating text, key/value fields lose their pairing, and merged inline runs render as separate rectangles. Set to `false` to skip.

`DOCLING_CONTENT_LAYERS` defaults to `body,furniture,background` at this backend layer — a permissive default that keeps page headers/footers (Docling puts these on `FURNITURE`) and watermarks (on `BACKGROUND`) visible in predictions. The interface renders each layer with its own opacity so it's still obvious which items are chrome vs body. Set to a comma-separated subset (e.g. `body`) if you explicitly want to drop chrome, or `body,furniture,background,notes,invisible` for the full docling set.
