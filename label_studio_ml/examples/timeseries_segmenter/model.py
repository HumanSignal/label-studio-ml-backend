...
class TimeSeriesSegmenter(LabelStudioMLBase):
    ...
    def _read_csv(self, task: Dict, path: str, params: dict) -> pd.DataFrame:
        ...
        if params['time_col'] is None:
            params['time_col'] = 'time_autogenerated'
            first_channel_values = df[params['channels'][0]]
            df[params['time_col']] = range(len(first_channel_values))
    ...
    def _predict_task(self, task: Dict, model: TimeSeriesLSTM, params: Dict) -> Dict:
        ...
        df = self._read_csv(task, task["data"][params["value"]], params)
