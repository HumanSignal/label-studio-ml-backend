...
def validate_startup_environment():
    ...
    logger.info(f'SAM3 model: {model_name}, variant: {"PCS" if hints else "Tracker"}')
    try:
        from transformers import AutoConfig
        config = AutoConfig.from_pretrained(model_name)
        logger.info(f'SAM3 config loaded: {config.model_type}')
    except Exception as e:
        logger.warning(f'Could not pre-validate SAM3 config: {e}')
    ...
    if not ls_host:
        logger.warning('LABEL_STUDIO_HOST/LABEL_STUDIO_URL not set')
    else:
