...
def bad_prompt(query, texts):
    return (
        ...
        "3. For each TEXT you should define if it is a `positives` or `hard_negatives` or `neutral` label "
        "based on the relevance and output it as `label` field.\n"
        "5. Each TEXT starts with `===>`, a number and a newline, then a text fragment follows "
        "(e.g. `\n===> 1.\n this is text one\n`), "
