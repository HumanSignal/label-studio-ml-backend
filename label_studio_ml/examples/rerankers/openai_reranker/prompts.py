...
def classification_prompt(query, texts):
    return (
        ...
        "Choose the most relevant `positives` to include within this limit. \n"
        "Choose `hard_negatives` to include within this limit. While `hard_negatives` are optional, "
        "having `hard_negatives` will greatly improve the quality of the fine-tuned model. "
        "Structure hard negatives to be semantically similar, rather than obviously incorrect documents "
        "(e.g. we would not recommend mining hard negatives using BM25 or non-semantic approaches).\n"
